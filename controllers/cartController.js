...
const removeAllCartItems = async (req, res) => {
  try {
    ...
    if (cartItems.length === 0) {
      return res.status(400).send({ success: false, msg: "No items found in the cart" });
    }
    ...
    res.status(200).send({ success: true, msg: "All items removed from the cart" });
  } catch (error) {
    res.status(400).send({ success: false, msg: error.message });
  }
};

module.exports = { addToCart, getCartItems, removeCartItem, removeAllCartItems };
